                            ...
                            const formatLastActive = (t: Team) => {
                                 ...
                                 try {
                                     ...
                                     if (t.updatedAt?.toDate && typeof t.updatedAt.toDate === 'function') {
                                         ...
                                         date = new Date(t.updatedAt as any);
                                     }
                                            ...
                                            <td colSpan={7} className="bg-slate-50/70 px-8 py-4 border-t border-b border-slate-100">
                                                <div className="space-y-4">
                                                    ...
                                                    {decs ? (() => {
                                                        ...
                                                        const getNegotiatedCost = (baseCost: number, supplierId: string, isComponent: boolean, productId: 'techbook' | 'zroid' | 'itab') => {
                                                            const overrides = decs.supplierOverrides;
                                                            let cost = baseCost;
                                                            if (isComponent && overrides?.componentCosts?.[productId]?.[supplierId] !== undefined) {
                                                                cost = overrides.componentCosts[productId]![supplierId];
                                                            } else if (!isComponent && overrides?.finishedGoodsCosts?.[productId]?.[supplierId] !== undefined) {
                                                                cost = overrides.finishedGoodsCosts[productId]![supplierId];
                                                            }
